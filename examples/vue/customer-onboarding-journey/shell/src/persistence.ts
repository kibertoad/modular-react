...
import type {
  OnboardingInput,
  OnboardingState,
} from "@example-vue-onboarding/customer-onboarding-journey";
 ...
 *
 * `createWebStoragePersistence` handles SSR guards, JSON parse errors, and lazy
 * `Storage` resolution — see `@modular-vue/journeys`.
 */
 ...
 * the Home page to show "Resume" vs "Start" affordances before the user commits
 * to opening a tab. Routes through the adapter's `load` so this probe inherits
 * SSR safety, lazy-`Storage` resolution, and the corrupt-JSON cleanup.
 */
