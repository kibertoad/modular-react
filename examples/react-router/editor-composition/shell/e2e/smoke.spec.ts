...
// Smoke + behavioral coverage for `@modular-react/compositions` mounted
// inside a React Router shell. Mirrors the customer-onboarding journey
// e2e style: a page/console-error collector wraps every test so that
// runtime regressions (duplicate-module copies, hydration mismatch,
// uncaught render errors) surface as test failures regardless of which
// specific UI assertion happens to be running.
...
test("home page mounts the CompositionOutlet without runtime errors", async ({ page }) => {
  ...
  // Source zone resolves to `empty` initially → renders nothing inside
  // its container. The container itself is visible.
...
test("a contentful source-item click writes through to the inspector zone", async ({ page }) => {
  ...
  // The same selection survives swapping the integration: the
  // composition state outlives any one panel.
  await page.getByTestId("source-choice-strapi").click();
  await expect(page.getByTestId("strapi-panel")).toBeVisible();
  await expect(page.getByTestId("inspector-selected")).toContainText("entry-42");
...
test("returning to None empties the source zone but keeps inspector state", async ({ page }) => {
  ...
  await expect(page.getByTestId("contentful-panel")).toHaveCount(0);
  await expect(page.getByTestId("strapi-panel")).toHaveCount(0);
  // Inspector still shows the previously-selected item — the
  // composition's state outlives the source panel's mount.
  await expect(page.getByTestId("inspector-selected")).toContainText("entry-19");
