 ...
 * (portal, focus trap, Escape, scroll lock, a11y) for free.
 *
 ...
 * exists to provide.
 */
function SecurityReportWindow({ subject }: { subject: StepRef | null }) {
  return (
    <div data-testid="window-body-acme-security-report">
      <p style={{ marginTop: 0 }}>
        Compliance scan for <strong>{subject?.label ?? "the run"}</strong> — secrets last rotated 3
        days ago, no findings.
      </p>
    </div>
  );
}
