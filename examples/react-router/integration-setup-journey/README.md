# Integration Setup Journey — React Router
...
```bash
pnpm install
pnpm --filter "@example-rr-integration-setup/shell" dev
```

Open <http://localhost:5174>. Click **Start integration setup** to mount the journey, pick an integration, complete the configure step, and watch the resulting payload render below — the `kind` field tells you which dispatch branch executed.

## Run the e2e tests

```bash
pnpm --filter "@example-rr-integration-setup/shell" test:e2e
```
