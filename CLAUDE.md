...
The check only counts a package's _shipped_ files — `changedFilePatterns` in
`.changeset/config.json` subtracts tests, fixtures, snapshots, test-runner
config, `README.md` and `CHANGELOG.md`, so test-only PRs need neither a
changeset nor the label. Any other Markdown inside a package still counts as
shipped; documentation outside `packages/*` is never in a package to begin
with.
