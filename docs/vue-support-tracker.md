...
**PR-20 (M): `@vue-router-modules/core`.** Done.
New `packages/vue-router-core` (`@vue-router-modules/core`, `0.1.0`) with the repo's standard skeleton (`vite build` + `rolldown-plugin-dts`, `vitest`, `tsc --noEmit`). Depends on `@modular-frontend/core`, `@modular-vue/vue`, a `vue ^3.5` peer, and a `vue-router ^4.5` peer (D5).
