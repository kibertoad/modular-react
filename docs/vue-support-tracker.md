...
| D3  | Store story for Vue templates: core store vs Pinia                                                                                   | Scaffold with the core `createStore` (zustand-shaped, already the framework contract) and document Pinia interop in a guide section. Do not take a Pinia dependency in runtime packages.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | resolved: core `createStore` shipped and documented (PR-41); Pinia interop shipped as structural adapters that take no `pinia` dependency — `createPiniaStoreAdapter` (Store<T> ↔ Pinia, `@modular-vue/vue`) and `createPiniaJourneyPersistence` (`@modular-vue/journeys`) — plus the guide section in [journeys-vue.md](./journeys-vue.md)                                   |
...
| D5  | Minimum supported versions                                                                                                           | Vue ^3.5, vue-router ^4.5, Node 22+, aligned with the React 19 / Node 22 baseline.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | resolved, then revised: shipped Vue ^3.5 + vue-router ^4.5 as the original baseline; with the Nuxt 4 integration the whole Vue family moved to **vue-router ^5.0** (Nuxt 4 requires it) and Pinia interop was validated on **pinia 4**. The ^4.5 peer arm was dropped rather than left untested. README, getting-started guide, CLI scaffold, and package peer ranges updated |
| D6  | Is Nuxt in scope for 1.0?                                                                                                            | No. Ship SPA-first, gauge demand, then decide on PR-52.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | resolved: Nuxt was out of scope for 1.0 (SPA-first shipped); the Nuxt integration now ships separately as `@modular-vue/nuxt` **0.1.x, experimental** (PR-52), versioned independently of the 1.0 family                                                                                                                                                                      |
...
**PR-05 (S): CI, publish, and workspace plumbing for the new scopes.** Done.
Wired the two new scopes — `@modular-frontend/*` (the shared engines, D2) and the `@modular-vue/*` family — into the release and CI automation so they publish and get validated like the React families.

- **`publish.yml` (the real gap).** The publish workflow drives off a hardcoded `dorny/paths-filter` filter set + a `PKG_MAP` (filter-key → `dir:npmName`), and neither listed any new-scope package — so a merge that changed a Vue or engine package would bump/publish nothing. Added eleven filter entries and eleven `PKG_MAP` entries: `@modular-frontend/core` (`frontend-core`), `@modular-frontend/testing` (`frontend-testing`), `@modular-frontend/journeys-engine` (`journeys-engine`), `@modular-frontend/compositions-engine` (`compositions-engine`), and `@modular-vue/{vue,core,runtime,testing,journeys,compositions,cli}`. Each filter watches `src/**` + `package.json` + `README.md` (the new packages build to a gitignored `dist`, so `src/**` is the publish-surface trigger, matching the `pkg_journeys` shape rather than the older `lib/**` packages). The dynamic matrix, version-bump, provenance-publish, tag, and commit steps are all package-agnostic, so no other change was needed there.
- **`ci.yml` (one missing package).** The `changed-files-job` `PATH_TO_NAME` map already covered `packages/vue`, `vue-core`, `vue-runtime`, `vue-testing`, `vue-journeys`, `vue-compositions`, and all four `@modular-frontend/*` engines — but **`packages/vue-cli` was never added when PR-50 landed**, so `@modular-vue/cli` changes never spawned a build-matrix job. Added `["packages/vue-cli"]="@modular-vue/cli"`. The reusable `ci.common.yml` build/test is turbo-driven (`turbo run build/test --filter=<name>`), so it runs the new packages' real (non-empty) suites unchanged.
...
Acceptance: met. Simulating the `publish.yml` matrix builder with all eleven new filter keys marked changed emits a matrix of all eleven packages, and every `dir → npmName` entry was verified against the on-disk `package.json` `name`. All four workflow files parse as valid YAML. CI's per-package build/test now includes `@modular-vue/cli`.
...
**PR-50 (M): `@modular-vue/cli`.** Done.
New `packages/vue-cli` (`@modular-vue/cli`, `1.0.0`, binary `modular-vue`) — a
preset over the PR-04 `cli-core` interface, mirroring the react-router-cli
package skeleton (`tsc` build, `cli-testlab` + `vitest` tests). The preset
supplies Vue SFC template bodies (shell layout components, module pages, a
route-zone detail panel, `Home`), a core-`createStore` `storeFile` (D3),
`@modular-vue/journeys` journey definition + persistence bodies, the
`createSharedComposables` app-shared contract (with the `RouteMeta`
augmentation), and the `vue` / `vue-router` / `vue-tsc` `package.json` +
`tsconfig` bodies. `modular-vue init`/`create module`/`create store`/`create
journey`/`create catalog` all work, and the generated shell owns the router via
`createModularApp` with module routes grafted under a named `root` layout route.

The enabling `cli-core` change (the PR-04 follow-through): the preset gained a
`scaffold` block carrying the entry/view file extensions (`main.ts` + `.vue`
vs. `main.tsx` + `.tsx`) and optional overrides for the framework-neutral
`package.json` / `tsconfig` / root-`vitest.config.ts` bodies. `init.ts`,
`create-module.ts`, and `transform.ts` read the entry filename and the journeys
binding name from the preset instead of hardcoding `main.tsx` /
`@modular-react/journeys`. React presets set only `entryMain` / `viewExt`, so
`cli-core` still emits its React-family defaults byte-identically — locked in by
the existing react-router (16) and tanstack (11) full-tree snapshot tests, which
stay green.

Deviations from the plan, both forced by the framework:

- **Module route paths are absolute (`/dashboard`), not relative.** vue-router
  rejects relative top-level paths, and `@modular-vue/testing`'s `renderModule`
  mounts a module's `createRoutes()` output as a top-level router in isolation.
  An absolute parent path resolves both in isolation and when grafted under the
  shell's named `root` route (an absolute child still renders inside the
  parent's `<router-view>`).
- **`AppZones` values are typed `UiComponent`, not vue's `Component`.**
  `useZones<AppZones>()`'s `ZoneMapOf` constraint requires values assignable to
  the neutral `UiComponent` (`(props) => any | new (props) => any`); vue's
  `Component` union includes non-callable options objects and fails it, while a
  `<script setup>` SFC's default export satisfies `UiComponent`.

Acceptance: met. A generated project (`init` + `create module`/`store`/`journey
--persistence`) typechecks clean across all five packages (`vue-tsc --noEmit`)
and its module tests pass through `renderModule` (+ `@vitejs/plugin-vue` in a
generated root `vitest.config.ts`), verified end-to-end in the workspace. The
CLI's own suite is 13 tests including a full-tree snapshot.
...
**PR-52 (stretch, L): Nuxt module.** Done.
New `packages/vue-nuxt` (`@modular-vue/nuxt`, `0.1.0`, experimental) with the repo's standard skeleton (`vite build` + `rolldown-plugin-dts`, `vitest` + `happy-dom` + `@vue/test-utils`, `tsc --noEmit`). Depends on `@nuxt/kit` (build-time module glue) with `@modular-vue/runtime`, `vue`, `vue-router` peers.

Nuxt owns the Vue app and the vue-router instance, so the integration is the **router-owning** path (`registry.resolve()`), not framework mode. Two seams:

- `installModularApp(nuxtApp, registry, options?)` — the runtime core. Takes the structural `NuxtAppLike` (`{ vueApp, $router }` — a real `NuxtApp` satisfies it, so no `nuxt`/`@nuxt/schema` runtime dependency for this path), calls `registry.resolve({ router: nuxtApp.$router, ...options })` to graft module routes via `router.addRoute()` + install the auth guard, then installs the manifest on `nuxtApp.vueApp` and returns it. Forwards `parentRouteName` / `authGuard` / `providers` / `slotFilter` / `onModuleExit`.
- The Nuxt module (`defineNuxtModule` default export, `configKey: "modularVue"`) — the zero-config path. `nuxt.config.ts` points `modularVue.registry` at a registry (or factory) export; the module transpiles the package and injects a runtime plugin (via `addPluginTemplate`) that calls `installModularApp` and exposes the manifest as `$modular`. Only the serializable options travel through config; `buildModularPluginContents(options)` (the pure plugin-source builder) is unit-tested standalone.

Design notes / deviations:

- **`resolve()` not `resolveManifest()`.** Nuxt creates the router, so there is no host `createRouter({ routes })` to feed `resolveManifest().routes` into. `resolve()`'s runtime `router.addRoute()` is the exact fit. Under SSR the registry must be built **per request** (fresh app + router per request; `resolve()` is single-use), so both wiring paths use a registry factory — documented, and covered by a test asserting the single-use throw on a reused singleton.
- **First-paint routing.** Runtime `addRoute` serves client navigation immediately; a deep-linkable module subtree needs a Nuxt page to graft under (`parentRouteName` + a shell/catch-all page), the Nuxt analog of "route shape owned by the framework." Documented in `framework-mode-nuxt.md`.
- **Build externals.** `@nuxt/kit` and `@nuxt/schema` are external for both the JS bundle and the dts pass (the `NuxtModule<ModuleOptions>` annotation on the default export references `@nuxt/schema`; without externalizing it the dts plugin tries to bundle Nuxt's CommonJS type graph and fails).

No example app or CI example-shell entry ships in this PR (a Nuxt example would pull the full Nuxt toolchain into the examples workspace + CI matrix); the runtime installer's real behavior is covered by the package suite against a live Vue app + vue-router. `framework-mode-nuxt.md`, the README package/guides/status entries, and the `ci.yml` / `publish.yml` plumbing all land here.

Acceptance: met. 19 tests across `install.test.ts` (8: route grafting, context injection, `parentRouteName` nesting, auth guard, provider/`onModuleExit` forwarding, static slots, single-use throw), `module.test.ts` (8: plugin-source emission incl. factory-unwrap + JSON-escaping, module meta/defaults, `setup` transpile + `addPluginTemplate`), `index.test.ts` (2: barrel), and `install.test-d.ts` (2). Typecheck, `vite build` (JS + dts), and the full-workspace build pass; `@nuxt/kit` / `@nuxt/schema` stay unbundled. D6 resolved in favor of shipping the integration as experimental `0.1.x`.
...
| PR-05 | CI/publish plumbing, scope reservation                            | S    | D1                  | done                          |
...
| PR-50 | @modular-vue/cli                                                  | M    | PR-04, PR-40        | done                          |
