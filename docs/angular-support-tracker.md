...
Status: **Proposed.** PR-A01 (seam documentation) landed; PR-A10 (`@modular-angular/angular` part 1) landed; PR-A20 (`@angular-router-modules/core`) landed. The rest of the board remains gated on the demand signal. Last updated: 2026-07-17.
...
Delivered: new `packages/angular` (`@modular-angular/angular`, `0.1.0`), plain TS on the repo's rolldown (`vite build` + `rolldown-plugin-dts`) skeleton, `vitest` + `happy-dom`, `tsc --noEmit`. Depends on `@modular-frontend/core` plus an `@angular/core` peer spanning the AD2 matrix (`^20 || ^21 || ^22`); dev-installed against v22. Files, each the analog of the same-named React/Vue file:

- `store-signal.ts` (analog of React `context.tsx`'s `useSyncExternalStore` path / Vue `store-ref.ts`): `storeSignal` / `reactiveServiceSignal` seed a `signal()` from `getState`/`getSnapshot`, push snapshots from `subscribe`, and tear down via `DestroyRef.onDestroy`. The base signal always holds the full snapshot; selector variants layer a `computed()`, whose `Object.is` output equality gives selector equality (verified by an effect-count test). Runs in an injection context or takes `{ injector }`; `assertInInjectionContext` reports the NG0203-style early error.
- `context.ts` (analog of `createSharedHooks` / `createSharedComposables`): `createSharedInjectors` giving `injectStore`, `injectService`, `injectReactiveService`, `injectOptional`. Reactive accessors return `Signal`s; `injectService` returns the value. `requireDep` uses `in` (not truthy) so a falsy plain-service value is still returned.
- `scoped-store.ts`, `modules-context.ts`, `navigation-context.ts`, `slots-context.ts`: typed `InjectionToken`s (`SHARED_DEPENDENCIES`, `MODULES`, `NAVIGATION`, `SLOTS`, `RECALCULATE_SLOTS`) + `provide*` provider factories + `inject*` accessors. Contexts set once at resolve time return plain values; only genuinely reactive sources (`injectStore`/`injectReactiveService`/`injectOptional`/`injectScoped`/`injectSlots`) return signals, matching the PR-10 Vue rule.

Deviations from the plan, all forced by the framework:

- New internal `injection-context.ts` (no React/Vue analog): a `runInContext(options, debugFn, fn)` helper and `splitSelectorOptions` shared by every accessor. React hooks and Vue composables auto-run in their reactive context; Angular's `inject()` does not, so the injection-context guard + `{ injector }` escape hatch is centralized here rather than duplicated per file.
- The Vue `DynamicSlotsProvider` **component** becomes `provideDynamicSlots(config)`, a plain provider factory: it runs in an injection context, seeds a writable signal from `buildDepsSnapshot` + `evaluateDynamicSlots`, re-evaluates on the `SlotsSignal`, and tears down via `DestroyRef.onDestroy`. No `@Component` is needed for the dynamic-slots job in Angular, so it stays in this plain-TS package (AD3).
- The React/Vue `useScoped` composable is the `injectScoped` accessor; `useSlots`/`useRecalculateSlots` become `injectSlots`/`injectRecalculateSlots`.

Test-harness finding (informs PR-A02, not AD3): Angular's published packages are partial-Ivy, and vite/esbuild does not run the Angular Linker, so the zoneless TestBed suites `import "@angular/compiler"` in a setup file for JIT fallback and boot the non-dynamic `platformBrowserTesting()` (no zone.js). No Angular compilation is needed for this plain-TS package itself — only its tests instantiate TestBed.
Acceptance: met. 43 tests across 7 files (`store-signal.test.ts`, `context.test.ts`, `scoped-store.test.ts`, `modules-context.test.ts`, `navigation-context.test.ts`, `slots-context.test.ts`, `navigation-context.test-d.ts`) port the PR-10 Vue inventory case-for-case: context (store/service/reactive-service/optional incl. wrong-bucket hint and unregistered throws), the injection-context / "within a modular app" error cases, subscribe-on-create + unsubscribe-on-injector-destroy via a listener-counting store, selector equality via an effect counter and `TestBed.tick()`, scoped store, modules/navigation/slots contexts, `provideDynamicSlots` recompute-on-signal, and the typed-label / typed-meta navigation `.test-d.ts`. Package `test`, `typecheck`, and `vite build` (JS + dts) pass; externals (`@angular/core`, `@modular-frontend/core`) stay unbundled.
...
Delivered: new `packages/angular-router-core` (`@angular-router-modules/core`, `0.1.0`), plain TS on the repo's rolldown (`vite build` + `rolldown-plugin-dts`) skeleton, `tsc --noEmit`. Depends on `@modular-frontend/core` + `@modular-angular/angular` (workspace) plus `@angular/core` and `@angular/router` peers spanning the AD2 matrix (`^20 || ^21 || ^22`; dev-installed against v22). Files, each the analog of the same-named React source (`react-router-core/src`) and Vue analog (`vue-core/src`) where one exists:

- `types.ts` (analog of React/Vue `types.ts`): `ModuleDescriptor` re-narrows the base descriptor's `createRoutes` to `() => Route | Route[]` (Angular Router's `Route`), plus `LazyModuleDescriptor` and the bivariant `AnyModuleDescriptor<TNavItem>` shorthand, and re-exports the shared `@modular-frontend/core` types.
- `define-module.ts`, `define-slots.ts` (analogs of React/Vue): identity `defineModule` inference helper and the headless `defineSlots` shorthand, unchanged from the React/Vue bodies except for the router-narrowed descriptor type.
- `route-data.ts` (analog of Vue `route-meta.ts` and the React `handle` channel): `ModuleRouteData<TZones> = Partial<TZones> & Record<string, unknown>`, the convention for carrying zone components and per-route statics on a route's `data`. Angular Router's `Route['data']` is a single non-augmentable `Data` type (unlike vue-router's global `RouteMeta`), so the helper is applied per-route with `satisfies` at the authoring site, matching the React `handle` pattern; the PR-A23 runtime accessors will funnel `data` through core's `mergeRouteStaticData`.
- `index.ts` (analog of Vue `index.ts`): re-exports the detection helpers (`isStoreApi`/`isReactiveService`/`separateDeps`) from `@modular-frontend/core`, and the shared-injector context (`SHARED_DEPENDENCIES`/`provideSharedDependencies`/`createSharedInjectors`/`SharedDependenciesContextValue`) and scoped store (`createScopedStore`/`ScopedStore`) from `@modular-angular/angular` — mirroring how `@modular-vue/core` re-exports from `@modular-vue/vue`. `is-store-api.ts` and `scoped-store.ts` are therefore re-exports, not reimplementations (the engine and the PR-A10 binding already own them), the same shape the Vue core landed with.

To keep the two Angular packages on one patch, all workspace `@angular/*` resolutions were aligned to `22.0.7` in the lockfile (the existing `@modular-angular/angular` `^22.0.0` ranges are unchanged and its 46-test zoneless TestBed suite still passes on the bumped patch).
Acceptance: met. 12 tests across 3 files (`define-module.test-d.ts`, `route-data.test-d.ts`, `define-slots.test.ts`) port the Vue core inventory case-for-case: `createRoutes` narrowed to `Route | Route[]`, single-route acceptance, descriptor-generic preservation, typed i18n label + nav-meta pass-through, `AnyModuleDescriptor` bivariance, `LazyModuleDescriptor` default-export inference, the `ModuleRouteData` typed-zone / arbitrary-data cases, and the runtime `defineSlots` behavior. Package `test`, `typecheck`, and `vite build` (JS + dts) pass; externals (`@angular/core`, `@angular/router`, `@modular-frontend/core`, `@modular-angular/angular`) stay unbundled.
...
| ------ | --------------------------------------------- | ---- | ---------------------- | ------ |
| PR-A01 | Document UiComponent narrowing contract       | S    | none                   | done   |
| PR-A02 | Two-pipeline packaging spike                  | M    | none                   | todo   |
| PR-A03 | Outlet error-capture spike                    | M    | none                   | todo   |
| PR-A10 | @modular-angular/angular: store bridge and DI | M    | PR-A01                 | done   |
...
| PR-A20 | @angular-router-modules/core                  | M    | PR-A10                 | done   |
