...
Beyond the literal multi-module case, the same abstraction fits any flow whose state machine must own a "wait for an async backend event, advance exactly once" transition — websocket / SSE / push-notification waits with a polling fallback and a timeout. The obvious shortcut is a `useEffect(() => navigate(...), [data, ...])` hook in shell code, and it works until it doesn't: router commits can churn the `navigate` reference back through the effect's dep array, "timed out" flags leak across re-submissions, and side effects inside `refetchInterval` run during react-query's render-phase scheduling. The journey shape gives each of those a cleaner home — the latch lives inside the step's `useWaitForExit`, the subscribe/poll work lives inside its effect rather than a query callback, and each push of the step starts with a fresh latch. See [Pattern - event-driven wait with timeout](../packages/journeys/README.md#pattern---event-driven-wait-with-timeout).
...
- [Journeys](../packages/journeys/README.md): typed multi-module workflows with serializable shared state — entry/exit contracts, pure transitions, pluggable persistence.
