 ...
 * vue-router-specific createRoutes that returns RouteRecordRaw(s).
 */
...
> extends Omit<BaseModuleDescriptor<TSharedDependencies, TSlots, TMeta, TNavItem>, "createRoutes"> {
  /**
   * Returns the module's route subtree as vue-router RouteRecordRaw(s).
   *
   * The runtime registers these via `router.addRoute()`, so a module can
   * contribute its routes at registration time without freezing the tree
   * up front.
   *
   * Optional — omit for "headless" modules that contribute only
   * via slots, navigation, and lifecycle hooks without owning routes.
   */
  readonly createRoutes?: () => RouteRecordRaw | RouteRecordRaw[];
}
 ...
 *
 * vue-router's `router.addRoute()` lets the runtime graft the loaded module's
 * route subtree in on first visit, so — unlike the frozen-tree routers — a Vue
 * lazy module contributes a full `createRoutes()` subtree, not just a single
 * catch-all component. See the base
...
/**
 * vue-router-narrowed {@link ModuleDescriptor} with every generic
 ...
 * filler. Preserves the vue-router-specific `createRoutes` signature
