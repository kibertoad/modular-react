---
name: scaffolding
description: Use when a consumer asks to scaffold or extend a TanStack Router modular-react workspace with @tanstack-react-modules/cli: init, create module, create store, create journey, generated workspace wiring, or CLI-first fixes.
sources:
  - ../../README.md
...
# TanStack Router CLI scaffolding

Prefer `@tanstack-react-modules/cli` for generated workspaces and routine additions. The CLI updates workspace files, package manifests, registry wiring, and shared types together.
...
```sh
npx @tanstack-react-modules/cli create module billing --route billing --nav-group finance
```

Add a store:

```sh
npx @tanstack-react-modules/cli create store auth
```

Add a journey:

```sh
npx @tanstack-react-modules/cli create journey customer-onboarding --modules profile,plan,billing --persistence
```
...
- Generated route integration follows TanStack Router file-based mode unless the app explicitly uses library-owned routing.
