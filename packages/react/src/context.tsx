...
import type { Store, ReactiveService } from "@modular-react/core";
 ...
 *
 * Works with both zustand stores and @modular-react/core's built-in createStore —
 * any object implementing the Store<T> interface is supported.
 *
 * @example
 * // In @myorg/app-shared:
 * import { createSharedHooks } from '@modular-react/react'
 ...
 * const user = useStore('auth', (s) => s.user)        // store → reactive
...
export function createSharedHooks<TSharedDependencies extends Record<string, any>>() {
  ...
  function useStore<K extends keyof TSharedDependencies & string>(
    ...
    if (!store) {
      ...
      if (hint) {
        throw new Error(`[@modular-react/react] "${key}" is not a store. ${hint}`);
      }
      throw new Error(
        `[@modular-react/react] "${key}" is not registered. ` +
  ...
  function useService<K extends keyof TSharedDependencies & string>(
    ...
    if (!service) {
      const hint = suggestHook(key, ctx);
      if (hint) {
        throw new Error(`[@modular-react/react] "${key}" is not a service. ${hint}`);
      }
      throw new Error(
        `[@modular-react/react] "${key}" is not registered. ` +
          `Available dependencies: ${allKeys(ctx)}`,
  ...
  function useReactiveService<K extends keyof TSharedDependencies & string>(
    ...
    if (!rs) {
      const hint = suggestHook(key, ctx);
      if (hint) {
        throw new Error(`[@modular-react/react] "${key}" is not a reactive service. ${hint}`);
      }
      throw new Error(
        `[@modular-react/react] "${key}" is not registered. ` +
          `Available dependencies: ${allKeys(ctx)}`,
  ...
  function useOptional<K extends keyof TSharedDependencies & string>(
    ...
    // Determine subscribe/getSnapshot once — the source never changes
    ...
    if (store || rs) {
      return reactiveValue as TSharedDependencies[K];
    }
