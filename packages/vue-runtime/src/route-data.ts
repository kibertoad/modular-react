 ...
 * the "non-component zone" escape hatch for vue-router.
 *
 * `useZones` is the component-typed channel: each value must be a
 * `UiComponent | undefined` so the shell can render it in a layout region.
 * That constraint is a useful rail 95% of the time, but it gets in the way
 * for non-component metadata a module wants to attach to a route — a header
 * variant enum, a page title string, an analytics event name, a per-route
 * feature flag. `useRouteData` is the relaxed-typing counterpart: same
 * deepest-wins merge over `route.matched[i].meta`, no constraint on values.
 *
 * Two composables, two channels: keep components in `meta` fields consumed
 * by `useZones`, keep metadata in fields consumed by `useRouteData`. They
 * can co-exist in the same `meta` object because they read the same match
 ...
 * }
 *
 * // Layout reads each channel with its own typing (`<script setup>`):
 * const { HeaderActions } = useZones<AppZones>().value
 ...
 *
 * The merged object is the raw merged `meta` — TypeScript narrows what you
 ...
 * `HeaderActions`) on the same `meta` object, it appears here too. Read by
 * declared key, not by `Object.keys()` iteration.
 *
 * ## Return value identity
 *
 * A `ComputedRef` is returned; it recomputes on navigation because
 * `route.matched` is reactive. The merge produces a fresh object each time
 * it recomputes — destructure the fields you need off `.value` rather than
 * treating the whole object as a stable identity.
 */
