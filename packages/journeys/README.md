...
- **State typing is on the author.** The module surface types `state` as `unknown` because modules don't know which journey hosts them. Annotating the parameter as `(state: ProjectState) => …` is the canonical pattern, but it gives no compile-time check that `ProjectState` actually matches the host journey's `TState` — a wrong annotation passes silently. If your `tsconfig` enables `strictFunctionTypes`, the parameter-narrowing TypeScript otherwise allows may produce a `Type '(state: ProjectState) => T' is not assignable to type '(state: unknown) => T'` error; widen the annotation back to `unknown` and cast inside the body, or wrap with an `as unknown as` at the `defineEntry` call site. A typed journey-side helper (e.g. `buildInputFor<TState>`) is a candidate follow-up.
