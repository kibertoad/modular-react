...
| `stepPathFromDefinition`      | `(definition, fallback?) => stepToPath` — builds a `stepToPath` from a journey's per-step `steps[module][entry].path` metadata, so a declared `JourneyStepMeta.path` drives the URL. Steps without a `path` fall through to `fallback` (default `defaultStepPath`). Pass the result as the sync's `stepToPath` to opt in.                                                                                                                                                                                                              |
