...
Wrap a handler with `defineTransition` to declare the entry points it can advance into. The runtime reads `targets` to **speculatively warm the chunks** for those entries during idle time after the current step mounts (see [`<JourneyOutlet preload>`](#props)). Bare-function handlers continue to work — they just contribute nothing to the precise-mode preload set.

The helper has two call shapes:

- **Curried (recommended)** — `defineTransition<TModules, TState>()` binds the journey's generics once. Handlers wrapped with the returned binder get `targets` autocompleted to `{ module, entry }` pairs from the journey's module map (typo on either field is a compile error) and the handler's `next` return contextually narrowed (so `module: "plan"` is checked against `keyof TModules`).
- **Bare** — `defineTransition({ targets, handle })` for one-off use. `targets` accepts any `{ module: string; entry: string }[]` (no autocomplete) and the handler's return is _not_ contextually narrowed. The runtime behavior is identical.

The `{ module, entry }` shape is the same `targets` and `next:` use everywhere else — `targets` is just `next:` minus the runtime-computed `input` field.
  ...
  transitions: {
    profile: {
      review: {
        ...
        // `targets` mirrors the `next:` shape — same `{ module, entry }`
        // pairs, minus the runtime-computed `input` field.
        ...
        // Bare function — still works, no preload contribution from this exit.
        cancelled: ({ state }) => ({ abort: { reason: "user-cancelled" } }),
      },
    },
  },
});
...
Why the explicit declaration? Targets are dynamic: handler bodies compute `next` from `state` / `output`, so the runtime can't statically know which entry will be picked without running the handler — and handlers may be impure. One declarative line per transition is the trade-off for precise preload.

**Type-level bonus.** With the curried form, `targets` autocompletes against the journey's `TModules`; renaming a module or entry surfaces every dangling target as a compile error. The same declaration unlocks future reachability lints ("dead target", "orphan step") without revisiting the API.

**Coverage.** `defineTransition` is purely additive — every existing bare-function handler keeps working unchanged. Migrate the handlers that fan out to heavy steps first; everything else stays as-is and falls back to either `preload="aggressive"` or no preload at all.
...
| `defineTransition`            | curried: `<TModules, TState?, TOutput?>() => (spec) => handle & { targets }`<br>bare: `<THandler, TTargets>(spec) => handle & { targets }` | Wraps a transition handler with declared `{ module, entry }` targets — same shape as `next:` minus `input`. The outlet's default `preload="precise"` mode reads these to warm exactly those chunks during idle time. Curried form (recommended) binds the journey's generics so `targets` autocompletes and `next` is contextually narrowed. Bare form is a zero-config drop-in. See [Pattern - declared targets](#pattern---declared-targets-with-definetransition-auto-preload--narrowed-return-type). |
