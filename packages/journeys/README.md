...
**Why a custom semver implementation?** The full `semver` package handles a much wider grammar than this use-case needs and parses regex-heavy on every call. The journeys-internal subset is ~5× faster on the cached path and ~1.7× faster on one-shot parses ([benchmark](src/semver.bench.ts)) while behaving identically to `semver.satisfies` on the syntax it does support — there is a fixture cross-check test that fails if the two ever disagree on a representative grid. If a range or version is outside the supported subset the parser throws synchronously, so an untested edge case fails loudly at registration rather than as a silent "no match."
