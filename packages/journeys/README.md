...
- **Curried (recommended)** — `defineTransition<TModules, TState>()` binds the journey's generics once. Handlers wrapped with the returned binder get `targets` autocompleted to `${moduleId}/${entryName}` literals and the handler's `next` return contextually narrowed against the journey's module map (so `module: "plan"` is checked against `keyof TModules`).
- **Bare** — `defineTransition({ targets, handle })` for one-off use. `targets` is typed `readonly string[]` (no autocomplete) and the handler's return is _not_ contextually narrowed, so heavy use of literal returns may need `as const` to keep the assignment to the `transitions` slot type-safe. The runtime behavior is identical.
