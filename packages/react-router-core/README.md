...
- **Types**: `ModuleDescriptor` (with React Router `RouteObject` support), `AnyModuleDescriptor` (router-narrowed shorthand for internal plumbing), `LazyModuleDescriptor`, `NavigationItem`, `ModuleLifecycle`, `ReactiveService`, `SlotMap`, `SlotMapOf`, `ZoneMap`, `ZoneMapOf`
