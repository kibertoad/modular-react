 ...
 * the templates emit, so don't remove that marker.
 */
export function addStoreToAppShared(
  ...
  if (content.includes(marker)) {
    ...
    content = content.replace(marker, storeInterface + marker);
  }

  // Insert the property into AppDependencies.
...
export function addJourneyToShellPackageJson(
  ...
  if (!pkg.dependencies["@modular-react/journeys"]) {
    pkg.dependencies["@modular-react/journeys"] = "^0.1.0";
  }
  pkg.dependencies = sortObject(pkg.dependencies);
 ...
 *   - import the handle + journey
 *   - install `journeysPlugin()` via `.use(...)` on the registry (idempotent)
 *   - call `registry.registerJourney(<journey>)` after module registrations
 */
export function addJourneyToMain(
  ...
  if (!content.includes("journeysPlugin(")) {
    content = content.replace(
      /const registry = createRegistry<([^>]+)>\(([\s\S]*?)\}\)(?!\.use)/,
  ...
  if (!content.includes(`registerJourney(${params.journeyExportName})`)) {
    ...
    if (insertAfter !== -1) {
      lines.splice(insertAfter + 1, 0, `registry.registerJourney(${params.journeyExportName})`);
      content = lines.join("\n");
    }
  }
...
export function ensureJourneysInWorkspace(projectRoot: string): void {
  ...
  // Insert under the existing `packages:` block, ordering after `modules/*`
  // (or before the trailing blank line / `onlyBuiltDependencies` block).
  const lines = content.split("\n");
  ...
  if (lastPackageIndex === -1) return;
  lines.splice(lastPackageIndex + 1, 0, "  - journeys/*");
